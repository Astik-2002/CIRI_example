    ...
    const double eps)
{
    ...
    for (int i = 0; i < n - 1; ++i) 
    {
        ...
        {
            std::cerr << "CIRI decomposition failed (serial)." << std::endl;
            continue;
        }
...
std::vector<Eigen::MatrixX4d> ObstacleFree::convexCoverParallel(const Eigen::MatrixXd &path, 
                                        const std::vector<Eigen::Vector3d> &points,
                                        const Eigen::Vector3d &lowCorner,
                                        const Eigen::Vector3d &highCorner,
                                        const double &range,
...
{
    int n = int(path.rows());
    std::vector<Eigen::MatrixX4d> hpolys(n - 1);
    if (n < 2) {
        std::cerr << "Path must contain at least two points." << std::endl;
        return hpolys;
    }
    ...
    for (int i = 0; i < n - 1; ++i) 
    {
        ...
        if (ciri.convexDecomposition(bd, pc, a, b, o1, tangent_obs2, false) != super_utils::SUCCESS) 
        {
            std::cerr << "CIRI decomposition failed (parallel)." << std::endl;
            continue;
        }
        geometry_utils::Polytope optimized_poly;
        ciri.getPolytope(optimized_poly);
...
*/
    ...
    if(ciri_testing)
    {
        ...
        std::uniform_real_distribution<> dis2(3.5, 5.0);
        ...
        Eigen::Vector3d min_pt{0.0, 0.0, 0.0};
        ...
        sfc_generator.convexCoverCIRI(path_rrt, demo_points, min_pt, max_pt, 2.0, _uav_size, CIRI_hpolys, o, tangent_obs, true);
        auto e1 = std::chrono::steady_clock::now();
        auto elapsed_ciri = std::chrono::duration_cast<std::chrono::milliseconds>(e1 - s1).count()*0.001;
        std::cout<<"[ciri debug] time taken in CIRI stochastic: "<<elapsed_ciri<<std::endl;
        std::vector<Eigen::MatrixX4d> CIRI_hpolys2;
        // sfc_generator.convexCoverParallel(path_rrt, demo_points, min_pt, max_pt, 2.0, _uav_size, o, tangent_obs, true);
        auto s2 = std::chrono::steady_clock::now();
        sfc_generator.convexCoverCIRI(path_rrt, demo_points, min_pt, max_pt, 2.0, _uav_size, CIRI_hpolys2, o, tangent_obs, false);
        auto e2 = std::chrono::steady_clock::now();
        auto elapsed_ciri2 = std::chrono::duration_cast<std::chrono::milliseconds>(e2 - s2).count()*0.001;
        std::cout<<"[ciri debug] time taken in CIRI deterministic: "<<elapsed_ciri2<<std::endl;
        CIRI_hpolys.push_back(CIRI_hpolys2[0]);
        // std::cout<<"CIRI_hpolys[0]: "<<std::endl;
        // std::cout<<CIRI_hpolys[0]<<std::endl;
        // std::cout<<"CIRI_hpolys_parallel[0]: "<<std::endl;
        // std::cout<<hpolys_parallel[0]<<std::endl;
        pcl::visualization::PCLVisualizer::Ptr viewer(new pcl::visualization::PCLVisualizer("Polytope Visualization"));
        pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud(new pcl::PointCloud<pcl::PointXYZRGB>());
        std::vector<pcl::Vertices> polygons;

        for (int i = 0; i < CIRI_hpolys.size(); i++) 
        {
            auto hPoly = CIRI_hpolys[i];
            Eigen::Matrix<double, 3, -1, Eigen::ColMajor> vPoly;
            geo_utils::enumerateVs(hPoly, vPoly);
        ...
        // std::cout<<"[ciri debug] alloc check 1"<<std::endl;
        pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud2(new pcl::PointCloud<pcl::PointXYZRGB>());
        std::vector<pcl::Vertices> polygons2;

        for (const auto &hPoly : CIRI_hpolys2) 
        {
            Eigen::Matrix<double, 3, -1, Eigen::ColMajor> vPoly;
            geo_utils::enumerateVs(hPoly, vPoly);
        ...
        // Validate that all planes are tangential to the ellipsoids
...
*/
        ...
        std::cout<<" time in firi (in seconds): "<<elapsed_firi<<" time in gcopter (in seconds): "<<elapsed_gcopter<<std::endl;
        ...
        if (traj.getPieceNum() > 0)
        {
            ...
            std::cout<<"ciri hpolys deterministic size: "<<CIRI_hpolys_deterministic.size()<<std::endl;
