...
double safeRegionRrtStar::computeThetaY(const Eigen::Vector3d& point) {
...
Eigen::Vector3d safeRegionRrtStar::computeNoiseStd(const Eigen::Vector3d& point) {
    ...
    double sigma_x = std::min(0.001063 + 0.0007278 * x + 0.003949 * x * x, 0.6);
...
// Eigen::Matrix3Xd safeRegionRrtStar::computePointCloudNoise(const Eigen::Matrix3Xd& pointCloud, const Eigen::Vector3d &o) {
...
{     
    ...
    double search_margin_local = std_dev_vector.maxCoeff()*sqrt(11.3345);
    double radius = sqrt(pointRadiusSquaredDistance[0]) - search_margin_local;
    return min(radius, double(max_radius));
}

inline double safeRegionRrtStar::radiusSearch_skeleton(Vector3d &search_pt)
{
    pcl::PointXYZ searchPoint;
    searchPoint.x = search_pt(0);
    searchPoint.y = search_pt(1);
...
std::vector<int> safeRegionRrtStar::getNeighbour(Eigen::Vector3d &pt)
{
    pcl::PointXYZ searchPoint;
    searchPoint.x = pt(0);
    searchPoint.y = pt(1);
    ...
    kdtreeForMap.nearestKSearch(searchPoint, 10, pointIdxRadiusSearch, pointRadiusSquaredDistance);
    std::vector<int> ans;
    for(int i = 0;i < 10; i++)
    {
        ans.push_back(pointIdxRadiusSearch[i]);
    }
    return ans;
...
inline NodePtr safeRegionRrtStar::genNewNode( Vector3d & pt_sample, NodePtr node_nearst_ptr )
{
    ...
    NodePtr node_new_ptr = new Node( center, radius_, INFINITY, h_dis_, 1.0); 
...
//  1 indicate node_1 contains node_2, node_2 should be deleted. 
    ...
    if( (dis + node_2->radius) == node_1->radius)
        status = 1;
    else if( (dis + 0.1) < 0.75 * (node_1->radius + node_2->radius) ) // && (dis > node_1->radius) && (dis > node_2->radius)
        status = -1;
    else
        status = 0;
...
{   
    ...
    while( ptr != NULL ) {
        std::cout<<"point: "<<ptr->coord[0]<<" : "<<ptr->coord[1]<<" : "<<ptr->coord[2]<<" radius: "<<ptr->radius<<std::endl; 
...
{   
    ...
    root_node = new Node(start_pt, radiusSearch( start_pt ), 0.0, min_distance, 1.0); 
