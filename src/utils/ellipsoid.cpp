...
bool Ellipsoid::intersectsPlane(const Eigen::Vector4d &plane) const {
    // Extract plane normal and offset
    Eigen::Vector3d n = plane.head(3);
    double b = plane(3);

    // Compute distance from ellipsoid center to plane
    double d_plane = (n.dot(d_) - b) / n.norm();

    // Compute maximum extent of ellipsoid along the plane normal
    Eigen::Vector3d n_normalized = n / n.norm();
    double r_max = (r_.asDiagonal() * R_.transpose() * n_normalized).norm();

    // Check if the plane passes through the ellipsoid
    return std::abs(d_plane) <= r_max - 1e-4;
}
